...
Zafu gives every transfer one final address check before you send. It compares copied vs pasted addresses, looks for poisoned lookalikes and known warning signals, shows stablecoin route context when available, displays the full address in readable chunks, and keeps the decision in your hands.
...
| **KNOWN** | Exact match in your trusted history | Low-friction banner with the full address and saved route context |
...
Core poisoning, copy-mismatch, address-family, and paste-time TRON checks run locally. Network-backed checks include GoPlus, chain-history providers, ENS resolution, Tronscan history import/review for TRON stablecoin routes, and the optional community threat list.
...
| `lib/tronscan-client.js` | Tronscan TRC-20 history wrapper for reviewed TRON imports |
