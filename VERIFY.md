...
# Verify the specific release tag
git checkout v1.1.0
...
# Package
cd extension
zip -r ../zafu-v1.1.0-local.zip . --exclude "*/.DS_Store"

# Compare SHA-256 against the release artifact
sha256sum zafu-v1.1.0-local.zip
...
Zafu requests exactly **4 Chrome permissions**. The average extension requests 17.
...
| `tabs` | Opens Etherscan and GitHub in a new tab from the popup | Cannot read tab URLs or content |
...
- **Zero telemetry** — no analytics, error reporting, or usage tracking
- **No external dependencies** — no npm, no bundler, no CDN scripts
- **Strict CSP** — no `eval`, no remote scripts, no inline code execution
- **Offline-first detection** — POISONED and HIJACKED checks run entirely locally
