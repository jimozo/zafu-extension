...
// 24h auto-refresh: re-scan previously-fetched wallets for fresh stats + suspicion signals.
// Runs in PREVIEW mode so it NEVER silently auto-trusts counterparties — wallet-history import
// is review-before-save, and a background refresh must not merge (or re-merge) recipients the
// user chose to skip. Saved contacts persist independently; only stats + suspicion are updated.
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name !== 'auto-refresh') return;
  const wallets = await getWallets();
  for (const wallet of wallets) {
    if (!wallet.lastFetchedAt) continue;
    await handleFetchHistory(wallet.id, wallet.address, wallet.chains || [wallet.chainId || 1], { preview: true }).catch(() => {});
  }
  ...
  if (msg.type === 'SUBMIT_COMMUNITY_REPORT') {
    ...
    Promise.all([getInstallId(), chrome.storage.local.get('settings')])
      .then(async ([installId, stored]) => {
        ...
        if (source !== 'user_flag' && stored.settings?.communityThreatSignals !== true) {
          sendResponse({ ok: true, submitted: false, reason: 'disabled' });
          return;
        }
        if (source === 'user_flag') bumpMetric('flag').catch(() => {});
        const result = await submitReport(msg.address, msg.chain, source, installId);
        ...
        sendResponse({ ok: true, submitted: result.ok, reason: result.reason || null });
      })
      .catch((err) => sendResponse({ ok: false, submitted: false, reason: err?.message || 'network' }));
    return true; // keep channel open for submission result
  }
...
async function handleStablecoinContactEnrichment(address) {
  ...
  if (network === 'tron' || isTronAddress) {
    ...
    // Recipient on-chain activity confirms the route or flags a mismatch, but is never counted
    // as the user's sends (see resolveEnrichmentConfidence).
    const confidence = resolveEnrichmentConfidence(entry, dominant, nextNetwork);
    await updateTrustedEntry(key, {
      asset: nextAsset,
      network: nextNetwork,
      ...confidence,
...
// Enrichment inspects the RECIPIENT's on-chain stablecoin activity, which is NOT the user's
// send history. It must never assert 'history'/"Sent N×" — those come only from wallet-history
// import (the user's own outgoing sends) and markTransferSent (self-attested receipts).
// Enrichment's honest jobs: (a) flag a saved-route vs on-chain-route mismatch, (b) preserve any
// real user-confirmed history untouched. The raw recipient activity is still retained on the
// entry as evidence (usdt/usdcActivityByChain), just never as a user-send count.
function resolveEnrichmentConfidence(entry, dominant, nextNetwork) {
  const userConfirmed = entry.networkConfidence === 'history';
  const historyFromSync = userConfirmed && entry.historyFromSync === true;
  const keepCount = Number(entry.dominantStablecoinTransferCount || 0);

  // On-chain dominant route differs from the saved route → genuine warning; surface the
  // on-chain route in the detail so the card can say "activity on <network>".
  if (dominant?.network && nextNetwork && nextNetwork !== dominant.network) {
    return {
      ...
      dominantStablecoinTransferCount: keepCount,
    };
  }

  // Preserve earned send history from this or another signed-in device. Sync provenance stays
  // explicit so this device still says "Checked route" rather than claiming it ran the scan.
  if (userConfirmed) {
    return {
  ...
  // No user sends yet: a usable saved route is 'saved' (renders "Needs test — send a small test
  // first"), never "Known recipient". Recipient activity stays only as raw evidence, count 0.
  return {
    networkConfidence: nextNetwork ? 'saved' : 'unknown',
    historyFromSync: false,
    dominantStablecoinAsset: entry.dominantStablecoinAsset || null,
    dominantStablecoinNetwork: entry.dominantStablecoinNetwork || null,
    dominantStablecoinTransferCount: keepCount,
...
async function submitSuspicionSignals(suspicionEntries) {
  ...
  // 'tron-dust' = zero-value inbound TRC-20 — the classic TRON address-poisoning bait, so it
  // belongs with the other high-signal attacker patterns fed to the community pool.
  const HIGH_SIGNAL_REASONS = new Set(['zero-value-token', 'solana-dust', 'inbound-or-zero-value', 'tron-dust', 'tron-zero-value']);
