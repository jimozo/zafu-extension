...
// Submit a community report. Reports require verified Google auth so a caller cannot mint
// arbitrary install IDs or high-weight sources to poison the shared warning list.
...
export async function submitReport(address, chain, source = 'user_flag', installId) {
  ...
  const token = await getGoogleAuthToken();
  if (!token) return { ok: false, reason: 'sign_in_required', retryable: false };
  try {
    ...
    if (res.ok) return { ok: true, reason: null, retryable: false };
...
export async function flushPendingReports(max = 20) {
  ...
  for (const r of batch) {
    const result = await submitReport(r.address, r.chain, r.source, r.install_id);
    if (result.ok) {
      sent++;
    } else if (result.reason === 'sign_in_required' || result.retryable) {
      remaining.push(r);
    }
  }
