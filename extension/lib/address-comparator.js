...
// EVM pipeline:
//   HIJACKED → EXCEPTION → MALICIOUS(curated) → FLAGGED(user-reported) → COMMUNITY_REPORTED(cross-user) →
...
import { getTrusted, getSuspicion, getExceptions, getFlagged, promoteFlaggedToMalicious, getCommunityList, getDisputedAddresses } from './storage.js';
 ...
 */
export async function compareAddress(pastedAddress, lastCopiedAddress) {
  await dataPromises;
  const chainType = detectChainType(pastedAddress);
  const normalized = normalizeAddress(pastedAddress, chainType);

  // 1. HIJACKED — clipboard diff. Chain-agnostic. Only compare when same chain family.
  if (lastCopiedAddress) {
    const copiedChain = detectChainType(lastCopiedAddress);
    if (copiedChain === chainType) {
      const normalizedCopied = normalizeAddress(lastCopiedAddress, copiedChain);
      if (normalized !== normalizedCopied) {
        return {
          state: 'HIJACKED',
  ...
  // 2. EXCEPTION — user manually verified.
