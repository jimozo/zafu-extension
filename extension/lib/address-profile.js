...
export async function enrichAddressProfile(profile, settings = {}) {
  ...
  try {
    ...
    // The GoPlus risk check is the only reputation signal on EVM. If it specifically rejected
    // (but other checks succeeded), the result is NOT a clean bill of health — mark it so the
    // record is stored as incomplete and re-run later instead of being cached as 'clear' forever.
    const riskUnavailable = source === 'etherscan' && checks[2]?.status === 'rejected';
...
// 'clear' (or 'risky'); 'incomplete'/'error'/'locked'/'needs_key' stay re-runnable.
function pendingIntelJobs(jobs, intelIndex) {
  return jobs.filter((job) => {
    const rec = intelIndex[`${normalizeKey(job.address)}:${String(job.chainId)}`];
    // Re-run when there is no record, when the last run failed, OR when it succeeded but its
    // risk check was unavailable (incomplete) — a cached gap must not become permanent.
    return !(rec && (rec.status === 'clear' || rec.status === 'risky') && rec.incomplete !== true);
  });
}
...
export async function runBulkAddressIntel(settings = {}, source = 'etherscan', onProgress = () => {}, { include = ['trusted', 'wallets'] } = {}) {
  ...
  for (const job of pending) {
    ...
    if (enriched.intel?.status === 'error' || enriched.intel?.status === 'incomplete') failed += 1;
    await sleep(350);
  }
...
function buildIntelRecord(profile, explorer) {
  ...
  return {
    ...
    explorer: {
      ...
      riskUnavailable: explorer.riskUnavailable === true,
...
function intelVerdict({ status, risk, contract, balance }) {
  ...
  if (status === 'incomplete') return 'Risk check incomplete';
