...
function mergeConfidence(existingValue, remoteValue) {
  // A route mismatch is a SAFETY WARNING (saved route conflicts with on-chain history). It must
  // never be silently cleared by sync — a stale 'history' from another device must not overwrite
  // a mismatch discovered here (or vice-versa). Mismatch wins over history from either side.
  if (existingValue === 'mismatch' || remoteValue === 'mismatch') return 'mismatch';
