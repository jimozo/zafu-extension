...
document.getElementById('add-wallet-form').addEventListener('submit', async (e) => {
  ...
  if (chainType === 'solana') {
    ...
    submitBtn.disabled = true;
    submitBtn.textContent = 'Adding…';
    statusEl.textContent = 'Detected TRON wallet. Adding local paste-time protection…';
    statusEl.classList.remove('hidden');
    activeChains = ['tron'];
    primaryChainId = 'tron';
  } else {
...
function buildAddressItem(entry, type, walletsMap = {}) {
  ...
  if (type === 'trusted') {
    ...
    profileBtn.className = 'btn-ghost small btn-profile';
    profileBtn.textContent = 'Intel';
    profileBtn.title = 'View address Intel';
...
async function loadProtectionStatus() {
  ...
  if (hasFetched) {
    el.textContent = `✓ Transfer Readiness — full protection, Telegram/TRON ready, ${network}`;
    el.className = 'protection-status protection-status--full';
  } else {
    el.textContent = `⚠ Transfer Readiness — base protection, Telegram/TRON ready, ${network}`;
