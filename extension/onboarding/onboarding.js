...
document.getElementById('wallet-address').addEventListener('input', (e) => {
  ...
  if (chainType === 'evm') {
    ...
    hint.textContent = 'TRON wallet — paste-time validation and trusted contact checks are local.';
  } else {
    pill.className = 'chain-pill hidden';
    hint.textContent = 'EVM (Ethereum, Polygon, Arbitrum, Base, Optimism, BNB), Solana, and TRON supported.';
  }
});

// --- Panel 2: Add wallet + fetch ---
...
document.getElementById('add-wallet-form').addEventListener('submit', async (e) => {
  ...
  if (chainType === 'solana') {
    ...
    activeChains = ['tron'];
    primaryChainId = 'tron';
    progressEl.textContent = 'Adding TRON wallet for local paste-time checks…';
    progressEl.classList.remove('hidden');
  ...
  if (chainType === 'tron') {
    ...
    const panel2SuccessMsg = document.getElementById('success-msg');
    if (panel2SuccessMsg) {
      panel2SuccessMsg.textContent = 'TRON wallet added. Zafu is watching your clipboard and Transfer Check supports TRON addresses locally.';
    }
    const inlineMsg = document.getElementById('fetch-success-msg');
    if (inlineMsg) {
      inlineMsg.textContent = 'Done — TRON wallet added for local paste-time protection.';
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      if (response && response.ok) {
        ...
        const panel2SuccessMsg = document.getElementById('success-msg');
        if (panel2SuccessMsg && trustedCount > 0) {
          panel2SuccessMsg.textContent = `${trustedCount} trusted addresses indexed from your history. Zafu is watching your clipboard.`;
        }
        const inlineMsg = document.getElementById('fetch-success-msg');
        if (inlineMsg) {
          inlineMsg.textContent = trustedCount > 0
            ? `Done — ${trustedCount} trusted addresses indexed from your history.`
            : 'Done — wallet added. Zafu is watching your clipboard.';
          inlineMsg.classList.remove('hidden');
        }
        doneBtn.classList.remove('hidden');
      } else {
