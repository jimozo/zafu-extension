  ...
  document.addEventListener(
    ...
    async (e) => {
      ...
      try {
        ...
        if (settings?.guardianMode) {
          // Pre-flight: show address (with ENS disclosure if applicable)
          const proceed = await overlay.showPreFlightConfirm(addressToUse, originalEnsName);
          if (!proceed) {
            console.log('[Zafu] paste cancelled at pre-flight');
            return;
          }
        }
      } catch (err) {
        console.warn('[Zafu] guardian mode check failed:', err);
      }
      ...
      const { confirmed, address } = await overlay.showOverlay(result, e.target);
...
function replayPaste(target, address) {
  ...
  if (isTextInput(target)) {
    ...
    const nextValue = target.value.slice(0, start) + address + target.value.slice(end);

    if (nativeInputSetter) {
      nativeInputSetter.call(target, nextValue);
    } else {
      target.value = nextValue; // fallback
    }
