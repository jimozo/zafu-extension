...
function reviewStatusForEntry(entry) {
  ...
  if (intel?.status === 'error') return 'failed';
  if (intel?.reviewedAt) return 'reviewed';
  if (entry._type === 'trusted') return 'needs-review';
...
function renderEmptyBookMessage(emptyMsg) {
  ...
  emptyMsg.innerHTML = `
    <div class="empty-msg-title">Start your trusted address book.</div>
    <p class="empty-msg-desc">Add a protected wallet and fetch its history to build trusted contacts automatically, or save a known recipient manually.</p>
    <div class="empty-msg-actions">
      <button id="book-empty-add-contact-inline-btn" class="btn-primary" type="button">+ Add contact</button>
      <button id="book-empty-add-wallet-inline-btn" class="btn-primary" type="button">+ Add protected wallet</button>
    </div>
  `;
  ...
  container.querySelectorAll('.btn-wallet-qr').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      showAddressQrModal({
        ...
        label: wallet.label || 'Protected wallet',
...
async function handleFetch(walletId) {
  ...
  if (walletChains.length === 1 && walletChains[0] === 'tron') {
    if (progressEl) {
      progressEl.textContent = 'TRON transaction-history sync is not included yet. Paste-time checks work locally.';
  ...
  chrome.runtime.sendMessage(
    { type: 'FETCH_HISTORY', walletId, address: wallet.address, chainIds: walletChains },
    async (response) => {
...
document.getElementById('add-wallet-form').addEventListener('submit', async (e) => {
  ...
  await renderWallets();
  if (chainType === 'solana' || chainType === 'tron') return;
  const wallets = await getWallets();
...
async function maybePromptBulkIntel(source, statusEl) {
  const flag = source === 'solscan' ? 'solscanIntelPrompted' : 'etherscanIntelPrompted';
  const settings = await getSettings();
  if (settings[flag]) return;
  await updateSettings({ [flag]: true });
  const sourceName = source === 'solscan' ? 'Solscan' : 'Etherscan';
  const confirmed = await showSimpleConfirm(
    `Run ${sourceName} Intel now?`,
    `ZAFU can review eligible trusted contacts and protected wallets with your local ${sourceName} key. Your key stays on this device.`
  );
  if (!confirmed) return;
  ...
  statusEl.textContent = `Intel done — ${result.completed} reviewed${result.risky ? `, ${result.risky} risk flagged` : ''}.`;
  await loadBookData();
  await renderWallets();
}
...
async function runManualBulkIntel(source) {
  const settings = await getSettings();
  const isSolana = source === 'solscan';
  const hasKey = isSolana ? !!settings.solscanApiKey : !!settings.etherscanApiKey;
  const statusEl = document.getElementById(isSolana ? 'solscan-key-status' : 'api-key-status');
  const sourceName = isSolana ? 'Solscan' : 'Etherscan';
  if (!hasKey) {
  ...
  const confirmed = await showSimpleConfirm(
    ...
    `Review eligible trusted contacts and protected wallets with your local ${sourceName} key.`
  );
  if (!confirmed) return;
  statusEl.textContent = 'Running Intel assessment…';
  statusEl.classList.remove('hidden');
...
async function showAddressBookIntelModal() {
  ...
  backdrop.innerHTML = `
    <div class="in-page-modal">
      ...
      <p class="in-page-modal-hint in-page-modal-hint--normal">
        Review saved trusted contacts and protected wallets with local explorer keys. Keys stay on this device.
      </p>
  ...
  `;
  const statusEl = backdrop.querySelector('#modal-intel-status');
  ...
  backdrop.querySelector('#modal-run-evm-intel').addEventListener('click', async () => {
    ...
    await runBulkIntelFromModal('etherscan', statusEl);
  });
  ...
  backdrop.querySelector('#modal-run-sol-intel').addEventListener('click', async () => {
    ...
    await runBulkIntelFromModal('solscan', statusEl);
  });

  document.body.appendChild(backdrop);
}

async function runBulkIntelFromModal(source, statusEl) {
  const settings = await getSettings();
  statusEl.textContent = 'Running Intel assessment…';
  const result = await runBulkAddressIntel(settings, source, (progress) => {
    if (progress.phase === 'running') {
  ...
  if (countEl) {
    ...
    if (countLabelEl) {
      countLabelEl.textContent = hasCount
        ? 'addresses checked during paste protection'
        : 'community protection feed';
    }
  }
...
async function showShareModal() {
  ...
  const tweetText = `Just installed @stayzafu — catches address poisoning and clipboard mismatches before you send. ${shareUrl}`;
...
async function showAddressProfileModal({
  ...
  const modalInput = { address, label, eyebrow, trustedEntry, protectedWallet, walletsMap };
  const settings = await getSettings();
  const localProfile = buildLocalAddressProfile({
    address,
    trustedEntry,
  ...
  backdrop.innerHTML = `
    <div class="in-page-modal profile-modal">
      ...
      <div class="in-page-modal-buttons">
        ...
        ${canRefresh
          ? '<button class="btn-confirm" id="profile-refresh-btn" type="button">Refresh Intel</button>'
  ...
  if (refreshBtn) {
    refreshBtn.addEventListener('click', async () => {
      ...
      refreshBtn.textContent = 'Refreshing Intel';
...
function trustName(trust) {
  const names = {
    protected_wallet: 'Protected wallet',
...
function canRefreshProfile(profile, settings) {
  if (profile.chainId === 'tron') return false;
...
function getProfileExplorerKeyState(profile, settings) {
  if (profile.chainId === 'tron') {
    return {
      ...
      actionText: 'TRON Intel not available',
      statusText: 'TRON support is local in this release: format validation, copy-vs-paste checks, Telegram source matching, and trusted contact comparison.',
      setupText: 'TRON Intel does not require an API key in v1.1.7.',
    };
  }
...
function explorerRows(profile, settings) {
  const source = profile.chainId === 'tron' ? 'Local TRON' : profile.chainId === 'solana' ? 'Solscan' : 'Etherscan';
