...
export async function showTransferCheck(result, targetElement, transferContext = {}, lastCopied = null) {
  return new Promise((resolve) => {
    ...
    const contextRows = buildTransferCheckRows(result, transferContext, copiedMatch);

    modal.innerHTML = `
      <div class="zafu-modal-header">
        <div class="zafu-state-badge zafu-state-badge--transfer">CHECK</div>
        <div>
          <div class="zafu-modal-title">${isUnknown ? 'Transfer Check' : 'Transfer Check passed'}</div>
          <div class="zafu-modal-subtitle">${isUnknown ? 'This address passed automated checks, but ZAFU has not seen it before.' : 'ZAFU checked this address before paste.'}</div>
        </div>
      </div>
      <div class="zafu-modal-body">
        ${displayLabel ? `<p class="zafu-modal-text">${escapeHtml(displayLabel)}</p>` : ''}
        <ul class="zafu-checklist">
          ${contextRows.map((row) => `
            <li class="zafu-checklist-row zafu-checklist-row--${row.kind}">
              <span class="zafu-checklist-mark">${row.mark}</span>
              <span>${escapeHtml(row.text)}</span>
            </li>
          `).join('')}
        </ul>
        <div class="zafu-address-display">${escapeHtml(segmentAddress(pastedAddress))}</div>
        <p class="zafu-modal-text muted">Review the amount, network, and final wallet confirmation before sending.</p>
        ${isUnknown ? `
          <div class="zafu-checkbox-group zafu-checkbox-required">
...
function buildTransferCheckRows(result, transferContext, copiedMatch) {
  const rows = [];
  const sourceRow = transferSourceRow(result.sourceEvidence);
  if (sourceRow) {
    rows.push(sourceRow);
  } else {
    rows.push(copiedMatch
      ? { kind: 'ok', mark: '✓', text: 'Matches what you copied' }
      : { kind: 'neutral', mark: '•', text: 'Copied-address match unavailable' });
  }
  rows.push({ kind: 'ok', mark: '✓', text: 'No poisoned lookalike detected' });
  rows.push({ kind: 'ok', mark: '✓', text: 'No known threat signal found' });
  rows.push(transferContext.recipientFieldDetected
    ? { kind: 'ok', mark: '✓', text: 'Recipient field detected' }
    : { kind: 'neutral', mark: '•', text: 'Address field context unclear' });
  if (transferContext.telegramWebDetected) {
    rows.push({ kind: 'ok', mark: '✓', text: 'Telegram Web address-only paste' });
  }
  if (transferContext.cryptoPageDetected) {
    rows.push({ kind: 'ok', mark: '✓', text: 'Crypto page context detected' });
  }
  if (transferContext.chainHint) {
    rows.push({ kind: 'neutral', mark: '•', text: `Nearby network hint: ${transferContext.chainHint}` });
  }
  if (result.state === 'UNKNOWN') {
    rows.push({ kind: 'neutral', mark: '?', text: 'Not in your trusted address book' });
  } else {
    rows.push({ kind: 'ok', mark: '✓', text: result.state === 'KNOWN_PUBLIC' ? 'Known public contract' : 'Known trusted address' });
  }
  rows.push({ kind: 'ok', mark: '✓', text: 'Full address shown below' });
  return rows;
}
...
function transferSourceRow(sourceEvidence) {
  if (!sourceEvidence || sourceEvidence.state === 'NO_RECENT_SOURCE') return null;
  const age = sourceEvidence.ageSeconds != null ? ` ${formatAge(sourceEvidence.ageSeconds)} ago` : '';
  if (sourceEvidence.state === 'MATCHED_TELEGRAM_SOURCE') {
    return { kind: 'ok', mark: '✓', text: `Copied from Telegram Web${age}. Pasted address matches.` };
  }
  if (sourceEvidence.state === 'MISMATCHED_TELEGRAM_SOURCE') {
    return { kind: 'warn', mark: '!', text: `Pasted address does not match the address copied from Telegram Web${age}.` };
  }
  if (sourceEvidence.state === 'SOURCE_EXPIRED') {
    return { kind: 'neutral', mark: '•', text: 'Telegram copy context expired. Review the full address before sending.' };
  }
  return null;
}
  ...
  modal.innerHTML = `
    ...
    <div class="zafu-modal-body">
      ...
      <div class="zafu-checkbox-group zafu-checkbox-required zafu-checkbox-warning">
        ...
        <label for="zafu-clipboard-mismatch-confirm" class="zafu-checkbox-label">I copied this from another source or verified it manually.</label>
      </div>
    </div>
    <div class="zafu-buttons">
      <button class="zafu-btn zafu-btn-secondary" data-action="cancel">Cancel</button>
  ...
  return new Promise((resolve) => {
    ...
    backdrop.innerHTML = `
      <div class="zafu-modal zafu-state-hijacked">
        ...
        <div class="zafu-modal-body">
          <p class="zafu-modal-text">ZAFU pasted the approved address, but the field no longer appears to contain that exact value.</p>
...
function showFlagToast() {
  ...
  toast.textContent = 'Report submitted for community review';
  toast.style.cssText = [
...
async function maybeShowSignInNudge() {
  ...
  nudge.textContent = 'Optional sign-in helps deduplicate future reports';
  nudge.style.cssText = [
